...
export const CountryInput = (props: { name?: string }) => {
	...
	async function addCountry(countryName: string) {
		...
		const { statusCode, countryData } = await useAPI(
			`country/${countryName}`,
			authToken,
		);
		...
		if (statusCode === 401) {
			alert(
				`An error has occurred. Please make sure you are logged in and try again.`,
			);
			return;
		}
		...
		if (statusCode === 400 || !countryData || countryData.length <= 0) {
			alert(
				`No data was found when searching for ${countryName}. Please check for any typos and try again.`,
			);
			return;
		}
	...
	return (
		<>
			...
			<Styled.CountryAddButton
				type="button"
				onClick={() => addCountry(countryValue)}
			>
				Add Country
			</Styled.CountryAddButton>
		</>
